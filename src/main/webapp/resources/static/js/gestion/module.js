app.controller('moduleController', function($scope, $http, $location, $compile) {
	...
	$scope.$watch('filtre.filiere', function(newValue, oldValue) {
		if(!angular.isUndefined(newValue) && newValue != null){
			$scope.table.ajax.url("/gestion/module/rest/list/"+newValue).load();
		}
		else{
			$scope.table.ajax.url("/gestion/module/rest/list/-1").load();
		}
	});
	
	$scope.init = function(){
		$scope.table = $('#moduleTable').DataTable( {
			"oLanguage" : language,
			"ajax": "/gestion/module/rest/list/-1",
			"columns": [
	...
	$scope.edit = function(id){
		$http.get("/gestion/module/rest/load/"+id).then(function (response) {
	...
	$scope.save = function(){
		...
		$http.post("/gestion/module/rest/save",$scope.dto).then(function (response) {
	...
	$scope.confirmRemove = function(id) {
		$http.get("/gestion/module/rest/delete/"+id).then(function (response) {
			$scope.table.ajax.reload();
		}, function error(response) {
